...
class ChildChunk(Base):
    ...
    summary_id = Column(UUID(as_uuid=True), ForeignKey("summary_documents.id", ondelete="SET NULL"), nullable=True, index=True)
    ...
    summary_document = relationship("SummaryDocument", back_populates="child_chunks")
    
    def __repr__(self):
        return f"<ChildChunk(id={self.id}, parent_id={self.parent_id}, summary_id={self.summary_id})>"
...
class SummaryDocument(Base):
    ...
    # Summary content
    summary_content = Column(Text, name="summary_content", nullable=False)
    ...
    child_chunks = relationship("ChildChunk", back_populates="summary_document")
